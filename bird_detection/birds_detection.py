...
def image_prediction(image_path, result_filename=None, save_dir="./image_prediction_results", confidence=0.5, model="./model.pt"):
    """
    Function to detect birds in images using a pre-trained YOLO model.

    Parameters:
        image_path (str): Path to the image file. Can be a local path or a URL.
        result_filename (str): If not None, this is the output filename.
    ...
    # Save the image if result_filename is provided
    if result_filename:
...
def video_prediction(video_path, result_filename=None, save_dir="./video_prediction_results", confidence=0.5, model="./model.pt"):
    """
    Function to detect birds in video frames using a trained YOLO model.

    Parameters:
        video_path (str): Path to the video file.
        result_filename (str): Output filename for annotated video.
        save_dir (str): Directory to save output files.
        confidence (float): 0-1, only results over this value are saved.
        model (str): Path to the YOLO model.
    ...
    try:
        ...
        model = YOLO(model)  # Load YOLO model
        ...
        while cap.isOpened():
            ...
            # Save the annotated frame to the output video file if needed
            if result_filename:
                out.write(frame)
        ...
        if 'cap' in locals() and cap is not None:
            cap.release()
        if 'out' in locals() and out is not None and result_filename:
            out.release()
